...
bool peer_identity_process(PeerData* v, const char* addr, bool is_banned, uint64_t timeout, bool do_timeout)
{
	...
	if (!v->in_game)
	{
		...
		server_send_msg(v->server, v->peer, CLRCODE_RED "olr/server~ v" STRINGIFY(BUILD_VERSION));
...
//Disaster Launcher
void launcher_tcp_thread(Server* server)
{
#ifdef _WIN32
	WSADATA wsaData;
	...
	while (server->running)
	{
		...
		int selectResult = select(tcp_sock + 1, &readfds, NULL, NULL, &tv);
		...
		buffer[0] = g_config.server_count;  // servers
		buffer[1] = 7;                       // players limit

		//info
		for (int i = 0; i < g_config.server_count && i < 10; i++)
		{
			Server* srv = disaster_get(i);
			if (srv)
			{
				int players = 0;
				MutexLock(srv->state_lock);
				players = server_total(srv);
				MutexUnlock(srv->state_lock);

				buffer[2 + i * 2] = (players > 0) ? 1 : 0;  // status
				buffer[3 + i * 2] = players;                  // players
			}
		}

		int sent = 0;
		while (sent < 64) {
			int n = send(client, (char*)buffer + sent, 64 - sent, 0);
			if (n <= 0) break;
			sent += n;
		}
...
bool server_worker(Server* server)
{
	...
	Thread tcp_thread;
	ThreadSpawn(tcp_thread, launcher_tcp_thread, server);
...
bool server_cmd_handle(Server* server, unsigned long hash, PeerData* v, String* msg)
{
	...
	switch(hash)
	{
		...
		case CMD_INFO:
		{
			...
			server_send_msg(v->server, v->peer, "-----------------------");
			server_send_msg(v->server, v->peer, CLRCODE_RED "olr" CLRCODE_BLU "server" CLRCODE_RST " v" STRINGIFY(BUILD_VERSION));
			server_send_msg(v->server, v->peer, "build from " CLRCODE_PUR  __DATE__ " " CLRCODE_GRN  __TIME__ CLRCODE_RST);
			server_send_msg(v->server, v->peer, msg);
			server_send_msg(v->server, v->peer, "-----------------------");
